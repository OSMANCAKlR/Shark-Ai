  ...
  const ShopAIChat = {
    ...
    UI: {
      ...
      setupEventListeners: function () {
        ...
        if (voiceButton && window.shopChatConfig?.voiceEnabled) {
          voiceButton.addEventListener("click", () => {
            ShopAIChat.Voice.toggleVoiceChat();
          });
        } else if (voiceButton) {
          // Hide voice button if voice is disabled
    ...
    API: {
      ...
      streamResponse: async function (
        ...
        try {
          ...
          const streamUrl = `${window.shopChatConfig?.apiBaseUrl || ""}/chat`;
      ...
      fetchChatHistory: async function (conversationId, messagesContainer) {
        try {
          ...
          const historyUrl = `${window.shopChatConfig?.apiBaseUrl || ""}/chat?history=true&conversation_id=${encodeURIComponent(conversationId)}`;
    ...
    /**
     * Voice chat functionality using LiveKit
     */
    Voice: {
      isConnected: false,
      isRecording: false,
      room: null,
      audioTrack: null,
      sdk: null, // Store the detected LiveKit SDK

      /**
       * Initialize LiveKit SDK
       */
      initSDK: function () {
        // Check for LiveKit SDK availability (UMD version uses LivekitClient global)
        if (typeof LivekitClient !== "undefined" && LivekitClient.Room) {
          this.sdk = LivekitClient;
          console.log("LiveKit SDK ready");
          return true;
        } else {
          console.error(
            "LiveKit SDK not found. Make sure the script is loaded.",
          );
          return false;
        }
      },
       ...
       */
      connectVoice: async function () {
        try {
          const livekitUrl = window.shopChatConfig?.livekitUrl;
          if (!livekitUrl) {
            console.error("LiveKit URL not configured");
            ShopAIChat.Message.add(
              "Voice chat is not properly configured.",
              "assistant",
          ...
          this.updateVoiceStatus("Connecting to voice chat...", "connecting");

          // Generate a room token for this conversation
          const conversationId =
            sessionStorage.getItem("shopAiConversationId") ||
            Date.now().toString();
          const token = await this.generateRoomToken(conversationId);
          ...
          // Create room
          this.room = new this.sdk.Room();

          // Set up event listeners
          this.room.on("participantConnected", (participant) => {
            console.log("Agent connected:", participant.identity);
            this.updateVoiceStatus(
              "Voice chat connected. You can start speaking!",
              "connected",
            );
          });
          ...
          this.room.on("dataReceived", (data, participant) => {
            ...
            try {
              ...
              if (message.type === "transcript") {
                // Show agent's response as text too
                ShopAIChat.Message.add(
                  message.text,
                  "assistant",
                  ShopAIChat.UI.elements.messagesContainer,
          ...
          this.updateVoiceButton("connected");
        } catch (error) {
          console.error("Voice connection error:", error);
          this.updateVoiceStatus("Failed to connect to voice chat", "error");
          ShopAIChat.Message.add(
            "Sorry, voice chat is not available right now. Please use text chat.",
            "assistant",
            ShopAIChat.UI.elements.messagesContainer,
          );
        }
      ...
      disconnectVoice: function () {
        ...
        this.updateVoiceButton("disconnected");
      ...
      enableMicrophone: async function () {
        try {
          if (this.room) {
            ...
            if (this.sdk && this.sdk.createLocalAudioTrack) {
              this.audioTrack = await this.sdk.createLocalAudioTrack();
            } else {
              throw new Error(
                "createLocalAudioTrack function not found in SDK",
              );
            }
       ...
       */
      generateRoomToken: async function (conversationId) {
        try {
          // Call your backend to generate a token
          const response = await fetch(
            `${window.shopChatConfig?.apiBaseUrl || ""}/api/voice/token`,
            {
              method: "POST",
              headers: {
                "Content-Type": "application/json",
              ...
              body: JSON.stringify({
                conversation_id: conversationId,
                shop_domain: window.location.origin,
              }),
            },
          );
       ...
       */
      updateVoiceButton: function (state) {
        const voiceButton = ShopAIChat.UI.elements.voiceButton;
        if (!voiceButton) return;

        const micIcon = voiceButton.querySelector(".voice-icon-mic");
        const stopIcon = voiceButton.querySelector(".voice-icon-stop");

        voiceButton.className = "shop-ai-chat-voice"; // Reset classes
        ...
        switch (state) {
          ...
          case "recording":
            ...
            voiceButton.title = "Stop Voice Chat";
            break;
          case "disconnected":
          default:
            micIcon.style.display = "block";
            stopIcon.style.display = "none";
            voiceButton.title = "Start Voice Chat";
            break;
        }
      },
       ...
       */
      updateVoiceStatus: function (text, type = "listening") {
        const voiceStatus = ShopAIChat.UI.elements.voiceStatus;
        ...
        if (voiceStatus && statusText) {
          ...
          // Update waveform animation based on type
          const waveform = voiceStatus.querySelector(".voice-waveform");
          if (waveform) {
            waveform.style.display = type === "listening" ? "flex" : "none";
          }
        }
      },
