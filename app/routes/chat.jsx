...
async function handleChatSession({
  ...
  console.log(`🔍 MCP Debug - Shop ID: ${shopId}`);
  console.log(`🔍 MCP Debug - Origin Domain: ${originDomain}`);

  // If Origin is the dev server, derive the actual shop domain from shop ID
  let shopDomain = originDomain;
  if (
    originDomain &&
    (originDomain.includes("trycloudflare.com") ||
      originDomain.includes("localhost"))
  ) {
    console.log(
      `🔧 Dev environment detected (Origin: ${originDomain}), deriving actual shop domain...`,
    );
    console.log(`🔧 Using Shop ID: ${shopId}`);

    // Simple approach: if we know the shop ID is for ai-sandbox-voice-agent, hardcode it for now
    if (shopId === "68616388662") {
      shopDomain = "https://ai-sandbox-voice-agent.myshopify.com";
      console.log(`✅ Hardcoded shop domain: ${shopDomain}`);
    } else {
      try {
        // Use the shop ID to get the actual shop domain via Shopify API
        if (shopId) {
          console.log(`🔧 Querying Shopify API for shop ID: ${shopId}`);
          // Query Shopify Admin API to get shop domain
          ...
          console.log(`🔧 Shopify API response:`, body);
          const myshopifyDomain = body.data?.shop?.myshopifyDomain;

          if (myshopifyDomain) {
            shopDomain = `https://${myshopifyDomain}`;
            console.log(`✅ Derived actual shop domain: ${shopDomain}`);
          } else {
            console.warn(`⚠️ No myshopifyDomain found in response`);
          }
        } else {
        ...
        console.warn(
          `⚠️ Could not derive shop domain from shop ID ${shopId}:`,
          error.message,
        );
        // Fall back to a constructed domain if possible
        shopDomain = originDomain; // Keep the dev server for now
      }
    }
  }

  console.log(`🎯 Final shop domain for MCP: ${shopDomain}`);
